# 🚀 Deploy Rithm Clean Installer to Arc & Cobalt

## ✅ **Connectivity Status:**
- **Arc (192.168.68.146)**: ✓ Reachable (1ms)
- **Cobalt (192.168.70.35)**: ✓ Reachable (<1ms)
...
### Copy and paste this complete installation block:
```bash
# === ARC INSTALLATION ===
cat > /tmp/install_clean.sh << 'RITHM_INSTALLER_EOF'
#!/bin/bash
...
chmod +x /tmp/install_clean.sh
sudo ZABBIX_SERVER=192.168.70.2 HOSTNAME=Arc /tmp/install_clean.sh
```

---

## 📋 **Step 2: Deploy to Cobalt**
...
### Run the same installer (just different hostname):
```bash
# Download and run (or copy the same script from Arc)
sudo ZABBIX_SERVER=192.168.70.2 HOSTNAME=Cobalt /tmp/install_clean.sh
```
...
1. Copy `Template_Rithm_Custom.json` to your Zabbix server
2. Zabbix web interface → **Configuration** → **Templates** → **Import**
3. Select `Template_Rithm_Custom.json` → **Import**

---

## 🔧 **Step 4: Add Hosts to Zabbix**

### Add Arc:
- Host name: `Arc`
- Groups: `Linux servers`
- Interfaces: Agent, `192.168.68.146`, Port `10050`
- Templates: Link `Template Rithm Custom`
...
## ✅ **Step 5: Test from Zabbix Server**

SSH to Zabbix server (192.168.70.2) and run:
```bash
# Test basic connectivity
...
zabbix_get -s 192.168.70.35 -k custom.cpu.temperature
zabbix_get -s 192.168.70.35 -k custom.network.connections_established
```

---

## 🎉 **Expected Results:**
- ✅ Clean installation with consistent `custom.*` parameters
- ✅ 30+ monitoring metrics per server  
- ✅ Both servers use identical template
- ✅ Auto-discovery for disks, services, network interfaces
- ✅ Smart triggers and thresholds

Ready to start with Arc?